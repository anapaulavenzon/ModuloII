...
let tabelaRecados = document.querySelector(
  "#container-registros"
) as HTMLDivElement;
...
function apagarRecado(codigo: number) {
  let listaRecados: Recado[] = buscarRecadosNoStorage();
  let indiceEncontrado: number = listaRecados.findIndex(
    (recado) => recado.codigo === codigo
  );

  let confirma: boolean = confirm(
    `Você tem certeza que deseja excluir o recado ${codigo}?`
  );
